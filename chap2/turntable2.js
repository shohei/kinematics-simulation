...
var deltaOmega = 0.1;//回転盤回転速度の増分[rps]
var flagStart = false;
var flagStep = false;
var flagReset = false;
...
function webMain() 
{
  ...
  var animate = function()
  {
    ...
    if(flagStart)
    {
      ...
      var speed = mag(vVelT);
      var criticalSpeed = Math.sqrt(muS * gravity * radius);
      if(speed < criticalSpeed )//飛び出す前
      {
      　form2.speed.value = speed;
      　form2.omega.value = table.vOmega.z;
        rigid.vVel.copy(vVelT);//現在の接線方向速度
        //円盤上の物体を円盤とともに回転
        rigid.vOmega.z = table.vOmega.z;
...
function initObject()
{
  ...
  rigid.vOmega.z = 0.0;//[rad/s]
  rigid.mass = 1;
  rigid.calcMomentOfInertia();
  rigid.flagTexture = true;
  vObsPos = new Vector3(radius + 5, 0, 1);//観察者の初期位置(回転系2）
