...
function webMain() 
{
  ...
  var animate = function()
  {
    ...
    if(flagStart)
    {
	  ...
	  {
		if(i == 0)
          rigid[0].vAcc.y =  spring[0].constant * disp / rigid[0].mass - 2*gamma * rigid[0].vVel.y;//ばねによる力の方向がrigid[1]とは反対
		else
		  rigid[1].vAcc.y = -spring[0].constant * disp / rigid[1].mass - 2*gamma * rigid[1].vVel.y;
			
        rigid[i].vVel.y += rigid[i].vAcc.y * dt;
	    rigid[i].vPos.y += rigid[i].vVel.y * dt;
	  }	
	  spring[0].vPos.y = (rigid[1].vPos.y + rigid[0].vPos.y) / 2.0;
