...
function initObject()
{
  ...
  sm2.structK =  parseFloat(form2.constK.value);//構造バネのバネ定数
  sm2.shearK = parseFloat(form2.shearK.value);//せん断バネのバネ定数
  sm2.hingeK = 0;//parseFloat(form2.hingeK.value);//ヒンジバネ
  ...
  sm2.vPos = new Vector3(0, -0.5, sm2.height0);//最初の質点の位置
  sm2.initialize();
  
  sm2.pole[0].vEuler = new Vector3(90, 0, 0);
  sm2.pole[0].vPos = new Vector3(0, 0, sm2.height0);

  for(var i = 0; i < sm2.numPoint; i++) sm2.point[i].flagFixed = false;
  sm2.point[0].flagFixed = true;
  sm2.point[sm2.numCol * (sm2.numRow+1)].flagFixed = true;
  //フロア
  floor0 = new Rigid();
...
{
  windVelY = parseFloat(form2.winVelY.value);//風速
} 
