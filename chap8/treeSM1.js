...
function webMain() 
{
  ...
  sm = new SpringMassTree();//バネ質点モデルよる樹木オブジェクト
  ...
  var animate = function()
  {
    ...
    if(flagStart)
    {
      ...
      {
        var f = 0.05 * windVel * (1 - windGrad * sm.point[k].vPos.y);
        if(f < 0) f = 0;
        var wind = 0.3 * Math.sin(2.0 * Math.PI * windFreq * t);
		sm.point[k].vForce0.x = f * wind;
		sm.point[k].vForce0.y = f * (1 + wind);
      }
   
	  var tt = dt / 10.0;
      for(var j = 0; j < 10; j++) sm.motion(tt);
...
function initObject()
{
  ...
  sm.structK0 = sm.shearK = parseFloat(form2.constK.value);//せん断バネと構造バネは同じバネ定数としている
  ...
  sm.numTrunk = parseFloat(form2.trunk.value);
  sm.numBranch0 = parseFloat(form2.branch0.value);
  sm.numBranch = parseFloat(form2.branch.value);
  sm.vPos = new Vector3();//根の質点の位置
  sm.alpha = parseFloat(form2.alpha.value) * DEG_TO_RAD;
  sm.beta = parseFloat(form2.beta.value) * DEG_TO_RAD;
  sm.rate = parseFloat(form2.rate.value);
  sm.create();  
  sm.point[0].flagFixed = true;
  //フロア
  floor0 = new Rigid();
...
function onClickDisp()
{
  ...
  if(nn == 0)      sm.dispType = "SMM";
  else if(nn == 1) sm.dispType = "LP";
  else             sm.dispType = "MIX";
  display();
}   
