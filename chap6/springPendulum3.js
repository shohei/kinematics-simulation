...
function webMain() 
{
  ...
  var animate = function()
  {
    ...
    {
      //オイラー法
      var ss, cc, sp, cp, disp, r, h;

      var tt = dt / 1000;//間引き表示
      for(var i = 0;i < 1000; i++)
      {
        r = Math.sqrt(rigid[1].vPos.x * rigid[1].vPos.x + rigid[1].vPos.y * rigid[1].vPos.y);
        h = heightFix - rigid[1].vPos.z;
        ss = r / spring[0].len;//Math.sin(theta);
        cc = h / spring[0].len;//Math.cos(theta);
        sp = rigid[1].vPos.y / r;//sin(phi)
        cp = rigid[1].vPos.x / r;//cos(phi)
        disp = spring[0].len - spring[0].len0;

        //Euler 
        rigid[1].vAcc.x = -spring[0].constant * disp * cp * ss / rigid[1].mass;        
        rigid[1].vAcc.y = -spring[0].constant * disp * sp * ss / rigid[1].mass;
		rigid[1].vAcc.z =  spring[0].constant * disp * cc / rigid[1].mass - gravity;
        ...
        r = Math.sqrt(rigid[1].vPos.x * rigid[1].vPos.x + rigid[1].vPos.y * rigid[1].vPos.y);
        theta = Math.atan2( r, heightFix - rigid[1].vPos.z);
      }
      
      r = Math.sqrt(rigid[1].vPos.x * rigid[1].vPos.x + rigid[1].vPos.y * rigid[1].vPos.y);
      h = heightFix - rigid[1].vPos.z;
      ss = r / spring[0].len;//sin(theta)
      cc = h / spring[0].len;//cos(theta)
      sp = rigid[1].vPos.y / r;//sin(phi)
      cp = rigid[1].vPos.x / r;//cos(phi)
      if(flagLimit) {
        ...
        //長さ制限のときの修正
        rigid[1].vPos = new Vector3(spring[0].len * cp * ss, spring[0].len * sp * ss, heightFix - spring[0].len * cc);
      }
        
      rigid[1].vEuler = getEulerZ(rigid[1].vPos, new Vector3(0.0, 0.0, heightFix));//おもり自身の回転角
      spring[0].vPos = new Vector3(rigid[1].vPos.x / 2, rigid[1].vPos.y / 2,  (heightFix + rigid[1].vPos.z) / 2.0);
      spring[0].vEuler = rigid[1].vEuler;
...
function initObject()
{
  ...
  rigid[1].vVel.x = parseFloat(form2.velX0.value);
  //オブジェクト0(固定点）
  rigid[0].kind = "CUBE";
  rigid[0].vSize = new Vector3(0.2, 0.2, 0.2);
