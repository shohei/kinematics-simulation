...
function webMain() 
{
  ...
  var animate = function()
  {
    ...
    if(flagStart)
    {
      ...
      if(timeTrace == 0) 
      {
        tracePos[countTrace].copy(rigid.vPos);
        //if(!flagLinearStop) 
        countTrace ++;
...
function initObject()
{
  ...
  else if(kindNo == 2) rigid.kind = "CYLINDER";
  ...
  if(kindNo == 0 || kindNo == 2) rigid.vSize = new Vector3(DIA, DIA,DIA);
  rigid.vVel.x = 0.0;　
  ...
  trace.nSlice = 6;
  trace.nStack = 6;
　for(var i = 0; i < maxTrace; i++) tracePos[i] = new Vector3(1000,0,0);//初期値は遠方
