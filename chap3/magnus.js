...
function initObject()
{
  ...
  speed0 = parseFloat(form1.speed0.value);//初速度[m/s]
  angle0 = parseFloat(form2.angle0.value);//放射角度[deg]
  rigid.mass = parseFloat(form2.mass.value);
  rigid.vSize = new Vector3(DIA, DIA, DIA);
  if(kindNo == 0 || kindNo == 2) rigid.vSize = new Vector3(DIA, DIA,2* DIA);
  rigid.vVel.x = 0.0;　
  rigid.vVel.y = speed0 * Math.cos(Math.PI * angle0 / 180.0);//水平軸
  rigid.vVel.z = speed0 * Math.sin(Math.PI * angle0 / 180.0);//鉛直軸
  rigid.vEuler = new Vector3(90, 0, 30);
  rigid.q = getQFromEulerXYZ(rigid.vEuler);
  rigid.vOmega.x = parseFloat(form1.omegaX.value) * 2 * Math.PI;
  rigid.vOmega.y = parseFloat(form1.omegaY.value) * 2 * Math.PI;
  rigid.vOmega.z = parseFloat(form1.omegaZ.value) * 2 * Math.PI;
  rigid.nSlice = 8;//16;
  rigid.nStack = 8;//16;
  rigid.radiusRatio = 1.0;
...
function onClickMagnus()
{
  if(form2.Magnus.checked) flagMagnus = true;
  else flagMagnus = false;
  initObject();
}
