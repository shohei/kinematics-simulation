...
function webMain() 
{
  ...
  sm = new SpringMassWalk();//バネ質点モデルよる樹木オブジェクト
...
function initObject()
{
  ...
  sm.theta1 = parseFloat(form2.theta1.value);
  sm.theta2 = parseFloat(form2.theta2.value);
  sm.speed = parseFloat(form2.speed.value);//歩行スピード
  sm.dir = sm.dir0 = parseFloat(form2.direction.value) * DEG_TO_RAD;//進行方向
  sm.vPos = sm.vPos0 = new Vector3(0, -2, 0);
  sm.mode = 0;
  sm.create();  
...
function onClickType()
{
  ...
  if(nn == 0)      sm.actionType = 1;
  else if(nn == 1) sm.actionType = 2;
  initObject();
}   
