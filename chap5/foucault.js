...
var dt;
var viscous;//粘性抵抗係数
var heightFix = 2.2;//固定点の高さ
var length0;//振り子の長さ
var mass0;  //おもりの質量
var theta;  //振れ角(rad)
var omega;  //振り子の角速度
var count;  //かえる跳び法で必要な演算回数
var coordNo = 0;//座標系の種類
var PERIOD = 60;//回転座標系の回転周期
var ANGLE = 0.0;
var	OMEGA = 2.0 * Math.PI / PERIOD;//回転座標系の角速度
...
var flagTrace = false;
var tracePos0 = []; //軌跡配列
var maxTrace = 300; //軌跡点個数の最大値
var numTrace = 100;  //軌跡点の表示個数
var countTrace = 0; //軌跡点個数のカウン ト
var periodTrace = 0.02;//軌跡点を表示する時間間隔
...
function webMain() 
{
  ...
  var animate = function()
  {
    ...
    if(flagStart)
    {
      ...
      if(count == 0) omega += alpha * dt/2;//かえる跳び法
      else           omega += alpha * dt;
...
　　　form2.period.value = period.toString();
      ...
      if(timeTrace == 0) {
        if(coordNo == 0)//固定座標系
          ...
          tracePos0[countTrace] = rotZ_rad(rigid[0].vPos, -ANGLE);
        countTrace ++;
...
　PERIOD = parseFloat(form2.rotPeriod.value);;//回転座標系の周期
　ANGLE = 0.0;
　OMEGA = 2.0 * Math.PI / PERIOD;//回転座標系の角速度
...
{ 
  ...
  var vLightPos = new Vector3(light.pos0[0], light.pos0[1], light.pos0[2]);
  vLightPos = rotZ_rad(vLightPos, ANGLE);
  light.pos = [vLightPos.x, vLightPos.y, vLightPos.z, 1];
  ...
  if(coordNo == 0)
  {//固定座標系
    if(Math.cos(camera.theta * DEG_TO_RAD) >= 0.0)//カメラ仰角90度でﾋﾞｭｰｱｯﾌﾟﾍﾞｸﾄﾙ切替
  ...
  {//回転座標系
    var vPos = new Vector3(camera.pos[0], camera.pos[1], camera.pos[2]);
    var p = rotZ_rad(vPos, ANGLE);
    if(Math.cos(Math.PI * camera.theta /180.0) >= 0.0)//カメラ仰角90度でﾋﾞｭｰｱｯﾌﾟﾍﾞｸﾄﾙ切替
      vpMatrix.lookAt(p.x, p.y, p.z, camera.cnt[0], camera.cnt[1], camera.cnt[2], 0, 0, 1);
    else
	  vpMatrix.lookAt(p.x, p.y, p.z, camera.cnt[0], camera.cnt[1], camera.cnt[2], 0.0, 0.0, -1.0);

/*
    var a0 = Math.atan2(camera.pos[1], camera.pos[0]);
    var vCnt = new Vector3(camera.cnt[0], camera.cnt[1], camera.cnt[2]);
    var vPos = new Vector3(camera.pos[0], camera.pos[1], camera.pos[2]);
    var r = distance(vCnt, vPos);
    var x = r * Math.cos(a0+ANGLE);
    var y = r * Math.sin(a0+ANGLE);
    
 //   if(Math.cos(camera.theta * DEG_TO_RAD) >= 0.0)//カメラ仰角90度でﾋﾞｭｰｱｯﾌﾟﾍﾞｸﾄﾙ切替
    if(Math.cos(Math.PI * camera.theta /180.0) >= 0.0)//カメラ仰角90度でﾋﾞｭｰｱｯﾌﾟﾍﾞｸﾄﾙ切替
	  vpMatrix.lookAt(x, y, camera.pos[2], camera.cnt[0], camera.cnt[1], camera.cnt[2], 0.0, 0.0, 1.0);
    else
	  vpMatrix.lookAt(x, y, camera.pos[2], camera.cnt[0], camera.cnt[1], camera.cnt[2], 0.0, 0.0, -1.0);
*/
  }
  ...
  var angleLoc = gl.getUniformLocation(gl.program, 'u_angle');
  gl.uniform1f(angleLoc, ANGLE);
  var coordNoLoc = gl.getUniformLocation(gl.program, 'u_coordNo');
  gl.uniform1i(coordNoLoc, coordNo);
  n = initVertexPoints();
...
　//フロアは常に回転
  ...
  floor0.vEuler.z = ANGLE * RAD_TO_DEG;
  floor0.draw(gl, n);
...
function onClick_coord()
{
  var radio1 =  document.getElementsByName("radio1");
  for(var i = 0; i < radio1.length; i++)
  {
     if(radio1[i].checked) coordNo = i;
  }
  //initObject();
}
...
{
  light.pos0[0] = parseFloat(form2.lightX.value);
  light.pos0[1] = parseFloat(form2.lightY.value);
  light.pos0[2] = parseFloat(form2.lightZ.value);
  display();
}
...
function onClickReset()
{
  ...
  initObject();
  form1.time.value = "0";
  for(var i = 0; i < numTrace; i++) tracePos0[i].x = 1000;
  display();
}
...
function initVertexPoints() 
{
  ...
  for(var i = 0; i < len; i++) 
  {
//  if(coordNo == 1) tracePos0[i] = rotZ_rad(tracePos0[i], ANGLE);//回転系
