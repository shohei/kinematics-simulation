...
function webMain() 
{
  ...
  var animate = function()
  {
    ...
    {
      //Runge-Kutta
      var ss = Math.sin(theta1 - theta2);
	  var cc = Math.cos(theta1 - theta2);
      ...
      var qa = gl1 / aa;
      //角加速度
	  var alpha1 = pa * (omega1 * omega1 * cc + len21 * omega2 * omega2) 
			+ qa * (s1 - mu * cc * s2) - (viscous / mass1) * omega1;
	  var alpha2 = len12 * (ss * omega1 * omega1 - cc * alpha1) - gl2 * s2 - (viscous / mass2) * omega2;
      
      var k11 = alpha1 * dt;
	  var k21 = alpha2 * dt;
	  var omg11 = omega1+k11/2.0;
	  var omg21 = omega2+k21/2.0;
	  var k12 = (pa * (omg11 * omg11 * cc + len21 * omg21 * omg21) + qa * (s1 - mu * cc * s2) - (viscous / mass1) * omg11) * dt;
	  var k22 = (len12 * (ss * omg11 * omg11 - cc * alpha1) - gl2 * s2 - (viscous / mass2) * omg21) * dt;
	  var omg12 = omega1+k12/2.0;
	  var omg22 = omega2+k22/2.0;
      var k13 = (pa * (omg12 * omg12 * cc + len21 * omg22 * omg22) + qa * (s1 - mu * cc * s2) - (viscous / mass1) * omg12) * dt;
      var k23 = (len12 * (ss * omg12 * omg12 - cc * alpha1) - gl2 * s2 - (viscous / mass2) * omg22) * dt;
      var omg13 = omega1+k13;
      var omg23 = omega2+k23;
      var k14 = (pa * (omg13 * omg13 * cc + len21 * omg23 * omg23) + qa * (s1 - mu * cc * s2) - (viscous / mass1) * omg13) * dt;
	  var k24 = (len12 * (ss * omg13 * omg13 - cc * alpha1) - gl2 * s2 - (viscous / mass2) * omg23) * dt;
      //角速度
      omega1 += (k11 + 2.0 * k12 + 2.0 * k13 + k14) / 6.0;
	  omega2 += (k21 + 2.0 * k22 + 2.0 * k23 + k24) / 6.0;
      //振れ角
	  theta1 += omega1 * dt;
	  theta2 += omega2 * dt;

	  var r1 = length1 * s1;
	  var r2 = r1 + length2 * s2;
	  var h1 = heightFix - length1 * Math.cos(theta1);
	  var h2 = h1 - length2 * Math.cos(theta2);
      //おもり(yz平面で運動）
