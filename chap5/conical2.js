...
function webMain() 
{
  ...
  var animate = function()
  {
    ...
    {
      //オイラー法とかえる跳び法
      var alpha = - viscous * omega / mass0;
	  if(n_method == 1 && count == 0) omega += alpha * dt/2;//最初のかえる跳び
      else                            omega += alpha * dt;
	  phi += omega * dt;//rad
	  //半径
	  var c = gravity / (omega * omega); 
	  var hh = heightFix - c;//おもりの高さ
      if(length0 < c + 0.01) flagStart = false;
	  var rr = Math.sqrt(length0*length0 - c*c);
	  ...
	  //周期の計測
	  var e = 0.05;
	  if(rigid[0].vPos.x < e && rigid[0].vPos.x > -e &&
         rigid[0].vPos.y < rr + e && rigid[0].vPos.y > rr - e)
	  {
	    t2 = elapseTime;
		period = t2 - t1;
	  }
...
function initObject()
{
  ...
  omega = Math.sqrt(omega2);//omegaの初期値
  //偏角phiの初期値
  phi = Math.PI / 2.0;
