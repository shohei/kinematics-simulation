...
function initObject()
{
  ...
  deltaV = parseFloat(form2.deltaV.value);
  ...
  rigid[0].vSize.y = parseFloat(form2.size0Y.value);
  rigid[0].vSize.z = parseFloat(form2.size0Z.value);
  rigid[0].vEuler = new Vector3(0, 0, 0);
...
function onChangeVel(){
  deltaVel= parseFloat(form2.deltaV.value);
}
function onClickPosX0(){
  rigid[0].state = "FREE"; rigid[1].state = "FREE";
  rigid[0].vVel.x += deltaVel; 
}
function onClickNegX0(){ 
  rigid[0].state = "FREE"; rigid[1].state = "FREE";
  rigid[0].vVel.x -= deltaVel; 
}
function onClickPosY0(){ 
  rigid[0].state = "FREE"; rigid[1].state = "FREE";
  rigid[0].vVel.y += deltaVel; 
}
function onClickNegY0(){ 
  rigid[0].state = "FREE"; rigid[1].state = "FREE";
  rigid[0].vVel.y -= deltaVel; 
}

function onClickPosX1(){ 
  rigid[0].state = "FREE"; rigid[1].state = "FREE"; 
  rigid[1].vVel.x += deltaVel;
}
function onClickNegX1(){ 
  rigid[0].state = "FREE"; rigid[1].state = "FREE"; 
  rigid[1].vVel.x -= deltaVel; 
}
function onClickPosY1(){ 
  rigid[0].state = "FREE"; rigid[1].state= "FREE"; 
  rigid[1].vVel.y += deltaVel; 
}
function onClickNegY1(){ 
  rigid[0].state = "FREE"; rigid[1].state = "FREE"; 
  rigid[1].vVel.y -= deltaVel;
}
...
{//矢印キーが押されたときrigid[0]のx,y方向の速度を加える
  var keycode;
  if (e != null) { 
    keycode = e.which; 
  ...
  if(keycode == 37) rigid[0].vVel.y -= 1;
  else if(keycode == 39) rigid[0].vVel.y += 1;
  else if(keycode == 38) rigid[0].vVel.x -= 1;
  else if(keycode == 40) rigid[0].vVel.x += 1;
}*/
